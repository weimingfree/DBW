...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            课程号：<input name="selectsid" id="selectsid">
    ...
    <div id="table">
        <c:choose>
            <c:when test="${info.list.size()!=0}">
                <div id="top">
                        <%--新增--%>
                    <a href="${pageContext.request.contextPath}/admin/addCourseInfo.jsp">
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="one(${s.cno})">编辑
                                    </button>
                                    <button type="button" class="btn btn-warning" id="mydel"
                                            onclick="del(${s.cno})">删除
                                    </button>
                                </td>
                            </tr>
                        </c:forEach>
...
<script type="text/javascript">
    ...
    function del(sid) {
        ...
        if (confirm("确定删除吗")) {
            ...
            $.ajax({
                url:"${pageContext.request.contextPath}/courseInfo/delete.action",
                data:{"sid":sid},
                type:"post",
                dataType:"text",
                success:function (msg){
                    alert(msg);
                    $("#table").load("http://localhost:8080/admin/courseInfo.jsp #table");
                }
            });
        }
    }
    ...
    function one(sid) {
        //向服务器提交请求，传递商品id
        var str = "?sid="+sid;
        location.href = "${pageContext.request.contextPath}/courseInfo/one.action" + str ;
    }
</script>
<!--分页的AJAX实现-->
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        $.ajax({
            url:"${pageContext.request.contextPath}/courseInfo/ajaxSplit.action",
            data:{"spage":page},
            type:"post",
            success:function () {
                //重新加载显示分页数据的容器
    ...
    function condition(){
        ...
        $.ajax({
            type:"post",
            url:"${pageContext.request.contextPath}/courseInfo/condition.action",
            data:{"sid":sid},
            success:function (){
                //刷新显示数据的容器
                $("#table").load("http://localhost:8080/admin/courseInfo.jsp #table")
