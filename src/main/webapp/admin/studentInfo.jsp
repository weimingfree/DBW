...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            学生学号：<input name="selectsid" id="selectsid">
    ...
    <div id="table">
        <c:choose>
            <c:when test="${info.list.size()!=0}">
                <div id="top">
                    <%--新增--%>
                    <a href="${pageContext.request.contextPath}/admin/addStudentInfo.jsp">
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="one(${s.sno})">编辑
                                    </button>
                                    <button type="button" class="btn btn-warning" id="mydel"
                                            onclick="del(${s.sno})">删除
                                    </button>
                                </td>
                            </tr>
                        </c:forEach>
...
<script type="text/javascript">
    ...
    function del(sid) {
        ...
        if (confirm("确定删除吗")) {
            ...
            $.ajax({
                url:"${pageContext.request.contextPath}/studentInfo/delete.action",
                data:{"sid":sid},
                type:"post",
                dataType:"text",
                success:function (msg){
                    alert(msg);
                    $("#table").load("http://localhost:8080/admin/studentInfo.jsp #table");
                }
            });
        }
    }
    ...
    function one(sid) {
        //向服务器提交请求，传递商品id
        var str = "?sid="+sid;
        location.href = "${pageContext.request.contextPath}/studentInfo/one.action" + str ;
    }
</script>
<!--分页的AJAX实现-->
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        $.ajax({
            url:"${pageContext.request.contextPath}/studentInfo/ajaxSplit.action",
            data:{"spage":page},
            type:"post",
            success:function () {
                //重新加载显示分页数据的容器
    ...
    function condition(){
        //取出查询的条件
        var sid = $("#selectsid").val();
        $.ajax({
            type:"post",
            url:"${pageContext.request.contextPath}/studentInfo/condition.action",
            data:{"sid":sid},
            success:function (){
                //刷新显示数据的容器
                $("#table").load("http://localhost:8080/admin/studentInfo.jsp #table")
